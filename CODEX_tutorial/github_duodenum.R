...
## Tcell location in villi
ggplot(cells %>% filter(ROI == "P9_Straight_villi_03") %>% filter(cell_type %in% c("epi_GLUT1", 
                                                                                   "epi_NDRG1", 
                                                                                   "epi_CA9", 
                                                                                   "Tcell_CD8", 
                                                                                   "Tcell_CD4")),
...
### Specific cell types
ggplot(cells %>% filter(cell_type %in% c("epi_GLUT1", "epi_NDRG1")), aes(x=centroid_x, y=centroid_y, color=cell_type)) +
  geom_point(size=0.1) +
  guides(colour = guide_legend(override.aes = list(size=6, alpha = 1))) +
  labs(color = "cell_type") +
  facet_wrap(~sample, scales = "free") +
...
##Alternative load the df
min_dist_to_epi_df <- readRDS("min_dist_to_epi_df.RDS")
...
min_dist_to_BM_df <- readRDS("min_dist_to_BM_df.RDS")
